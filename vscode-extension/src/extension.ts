...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    ...
    vscode.languages.registerCodeLensProvider({ pattern: '**/*.apix' }, codelensProvider)
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      'apix.runRequest',
      async (reqName: string, pathStr: string, method: string, rawBody?: string, expectedStatus?: number) => {
  ...
  // 4. Command: Copy cURL
  context.subscriptions.push(
    vscode.commands.registerCommand('apix.copyCurl', (pathStr: string, method: string = 'GET', rawBody?: string) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('apix.benchmark', (pathStr: string) => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('apix.openInTerminal', () => {
      const activeEditor = vscode.window.activeTextEditor;
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('apix.initProject', async () => {
...
`;
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('apix.switchEnv', async () => {
...
function executeHttpRequest(urlStr: string, method: string, body?: string): Promise<{ statusCode: number; statusMessage: string; headers: any; body: string }> {
  ...
  provideCodeLenses(document: vscode.TextDocument): vscode.CodeLens[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.includes('name:')) {
        ...
        if (m) currentReqName = m[1].replace(/["']/g, '').trim();
      }

      if (line.includes('method:')) {
        const m = line.match(/method:\s*([A-Z]+)/i);
        if (m) currentMethod = m[1].toUpperCase();
      }
...
class ApixTreeDataProvider implements vscode.TreeDataProvider<ApixTreeItem> {
  private _onDidChangeTreeData: vscode.EventEmitter<ApixTreeItem | undefined | null | void> = new vscode.EventEmitter<ApixTreeItem | undefined | null | void>();
  readonly onDidChangeTreeData: vscode.Event<ApixTreeItem | undefined | null | void> = this._onDidChangeTreeData.event;
  ...
  getTreeItem(element: ApixTreeItem): vscode.TreeItem {
    return element;
  }

  async getChildren(element?: ApixTreeItem): Promise<ApixTreeItem[]> {
    if (!element) {
      return [
...
class ApixTreeItem extends vscode.TreeItem {
  constructor(
    ...
    public readonly type: 'folder' | 'env' | 'endpoint',
